import graphene
from apps.routes.models import RouteMetadata, BucketListRoute, RouteGraphene
...
class RoutesQuery(object):
    ...
    owner_routes = graphene.List(RouteGraphene)
    bucket_list_routes = graphene.List(RouteGraphene)

    def resolve_route(self, info, pub_id, zoom):
      return RouteMetadata.objects.get(pub_id=pub_id)

    def resolve_routes(self, info, geohash, zoom, page, page_size):
      qs = RouteMetadata.objects.filter(geohash__startswith=geohash, is_public=True)
    ...
    def resolve_routes_search(self, info, search_text, limit=10):
      limit = min(limit, 10)
      return RouteMetadata.objects.filter(name__icontains=search_text, is_public=True)[:limit]
...
class RemoveOwnedRoute(graphene.Mutation):
    ...
    def mutate(self, info, route_pub_id):
        ...
        qs = RouteMetadata.objects.filter(pub_id=route_pub_id, owner_pub_id=user.pub_id)
        if not qs.exists():
            return RemoveOwnedRoute(ok=False)
